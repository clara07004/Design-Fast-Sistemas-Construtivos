...
**Três formas de obter as imagens.** A origem (IA × Drive) é **escolha sua** — perguntada ao final do briefing, ou no início do carrossel/estático no fluxo rápido. Nenhuma é default: a IA costuma integrar melhor à composição da copy (peça pensada como um todo); o Drive traz autenticidade de obra real. Decidir caso a caso.

**1. Fotos reais do Drive** — autenticidade de obra
Buscar nas pastas de produto do Google Drive (`_contexto/referencias.md`). São fotos reais de instalações e obras — autenticidade que IA não reproduz. Usar **só as curadas** (nunca a primeira; comparar; reprovar baixa qualidade). O Claude baixa, seleciona e usa diretamente.
