...
> A origem das imagens (foto real do Drive × imagem IA) é **escolha do usuário** — perguntada ao final do `/briefing-unity` (ou no início do carrossel/estático no fluxo rápido). Nenhuma é default: a IA costuma integrar melhor à composição da copy; a foto real do Drive traz autenticidade de obra (curar antes de usar). Decidir caso a caso.
...
**Os 3 estilos (todos com luz natural CLARA — a Fast não usa dark/moody):**
- `ambiente_pronto` — produto instalado em projeto acabado, luz natural ampla, ambiente premium
- `obra_em_acao` — profissional em ação (instalador em drywall/steel frame), luz de dia, real
- `detalhe_material` — macro do material/estrutura, cor fiel, técnico e claro
