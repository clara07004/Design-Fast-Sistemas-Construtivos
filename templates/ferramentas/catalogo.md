...
**O que faz:** Publicacao direta em Instagram Business/Creator e Facebook via API oficial
**Precisa de conta:** Sim, app no Meta for Developers + token de acesso
**Configurar:** Salvar `META_ACCESS_TOKEN` no `.env` e o IG User ID em `credentials/meta_ig_user_id.txt`
**Implementado em:** skill `publicar-social-unity` (`.claude/skills/publicar-social-unity/`)
**Endpoint:** `https://graph.facebook.com/v22.0` — imagem unica e carrossel
**Quando usar:** Publicar carrossel ou post estatico aprovado no Instagram

### Outras redes (referencia de endpoint)
...
**Observacao:** cada rede exige app/token proprio. Nunca cravar USER_ID, token ou ID de conta neste catalogo — usar sempre arquivos em `credentials/` (fora do git).
