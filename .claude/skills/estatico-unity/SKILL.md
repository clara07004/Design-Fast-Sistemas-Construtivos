...
npx.cmd playwright screenshot --viewport-size=1080,1350 "file:///CAMINHO/post-01.html" "CAMINHO/posts prontos/post-01.png"
```

Usar `npx.cmd` no PowerShell. Caminhos no `file:///` com barras normais `/`.

**O PNG final vai para a subpasta `posts prontos/`** (criar a pasta antes). O HTML de origem (`post-01.html`) e a imagem-fonte (`img-post.png`) **permanecem na pasta do dia** — o Playwright lê o HTML de lá (resolve `./img-post.png` e `../../../../marca/`) e grava só o PNG de saída em `posts prontos/`.
...
## Origem da imagem (verificar antes de produzir)

A origem da imagem de fundo é **escolha do usuário** — não é automática e não há default.
...
2. Se **não houver** origem registrada (pedido direto), **perguntar antes de gerar qualquer coisa:**

```
ORIGEM DA IMAGEM — como você quer a imagem de fundo deste post? (sempre 3 opções)
...
[3] Mista — combina foto real de fundo com elementos/composição gerados por IA
```

- **IA:** seguir o fluxo com `/gpt-image2-unity`.
- **Drive:** `search_files` na pasta de fotos (ID em `_contexto/referencias.md`) → **curadoria crítica** (`.claude/memory/feedback_curadoria_fotos.md`: nunca a primeira; comparar; reprovar baixa qualidade) → baixar só a aprovada, HEIC → JPG, usar como fundo.
- **Mista:** combinar a foto real curada (fundo) com elementos gerados por IA na composição; registrar a decisão.
...
5. Salvar o HTML em `conteudo/post-estatico/[periodo]/[dia]/post-01.html` e criar a subpasta `conteudo/post-estatico/[periodo]/[dia]/posts prontos/`

6. Renderizar via Playwright (o PNG final vai para `posts prontos/`):
```powershell
$env:PATH = [System.Environment]::GetEnvironmentVariable("PATH","Machine") + ";" + [System.Environment]::GetEnvironmentVariable("PATH","User")
npx.cmd playwright screenshot --viewport-size=1080,1350 "file:///CAMINHO/post-01.html" "CAMINHO/posts prontos/post-01.png"
```
...
conteudo/post-estatico/[periodo]/[dia]/
  img-post.png     ← foto gerada pelo GPT Image (imagem-fonte)
  prompt.txt         ← prompt da foto (referência para variações)
  post-01.html       ← layout montado
  posts prontos/     ← PNG final publicável (o que você seleciona pra postar)
...
- **Tamanho exato 1080×1350, sem exceção.** O HTML tem `width:1080px; height:1350px; overflow:hidden`
  ...
  ```powershell
  python ".claude/skills/publicar-social-unity/validar-dimensao.py" "CAMINHO/posts prontos/post-01.png" 1080 1350
  ```
...
- Se o usuário pedir versão story (9:16): adaptar viewport para 1080×1920 e salvar o PNG em `posts prontos/post-01-story.png`
