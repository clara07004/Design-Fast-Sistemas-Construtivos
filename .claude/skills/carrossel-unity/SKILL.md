...
npx.cmd playwright screenshot --viewport-size=1080,1350 "file:///CAMINHO_ABSOLUTO/instagram/slide-XX.html" "CAMINHO_ABSOLUTO/instagram/posts prontos/slide-XX.png"
```

**O PNG final vai para a subpasta `posts prontos/`** (criar a pasta antes: `posts prontos/` dentro de `instagram/`). O HTML de origem **permanece em `instagram/`** — o Playwright lê o HTML de lá (é isso que resolve `./img-slideXX.png` e `../../../../../marca/`) e grava só o PNG de saída em `instagram/posts prontos/`. Nunca mover HTMLs nem imagens-fonte para `posts prontos/`.
...
```powershell
# 1) Dimensão do PNG — o canvas tem que ser exatamente 1080×1350 (aponta para os PNGs finais)
python ".claude/skills/publicar-social-unity/validar-dimensao.py" "CAMINHO_ABSOLUTO/instagram/posts prontos" 1080 1350

# 2) Overflow interno — nenhum texto pode ultrapassar as bordas do canvas (aponta para os HTMLs)
...
## Origem das imagens (verificar antes de produzir)

A origem das imagens é **escolha do usuário** — não é automática e não há default.

1. Se veio do `/briefing-unity`, ler o campo **`Origem da imagem`** no `_briefing.md`/`_aprovado.md` e seguir a escolha registrada (IA ou Drive).
2. Se **não houver** origem registrada (fluxo rápido, direto pelo carrossel), **perguntar antes de gerar qualquer coisa:**

```
ORIGEM DA IMAGEM — como você quer as imagens deste carrossel? (sempre 3 opções)
[1] Imagem IA (via gpt-image2-unity) — integra melhor à composição da copy
[2] Foto real do Drive — acervo de obra real, com curadoria crítica
[3] Mista — combina IA e foto real (definir quais slides vêm de cada origem)
```

- **IA:** seguir o fluxo de prompts + `/gpt-image2-unity`.
- **Drive:** `search_files` na pasta de fotos (ID em `_contexto/referencias.md`) → **curadoria crítica** (`.claude/memory/feedback_curadoria_fotos.md`: nunca a primeira; comparar; reprovar baixa qualidade) → baixar só a(s) aprovada(s), HEIC → JPG, salvar como `img-slideXX.jpg`.
- **Mista:** definir quais slides usam IA e quais usam Drive; produzir cada um pelo fluxo correspondente e salvar todos como `img-slideXX.jpg`.
...
5. Criar a subpasta `conteudo/carrosseis/[periodo]/[dia]/instagram/posts prontos/`
6. Renderizar cada HTML em PNG via PowerShell (comando acima), gravando o PNG em `instagram/posts prontos/`
...
Salvar os PNGs finais em `conteudo/carrosseis/[periodo]/[dia]/instagram/posts prontos/` (o slide de CTA copiado de `CTA's/` também entra aqui, como último slide). Os HTMLs e as imagens-fonte `img-slideXX.png` ficam em `instagram/`.
...
- Salvar os HTMLs em `conteudo/carrosseis/[periodo]/[dia]/tiktok/` e os PNGs finais em `conteudo/carrosseis/[periodo]/[dia]/tiktok/posts prontos/`
...
conteudo/carrosseis/[periodo]/[dia]/
  ...
  instagram/
    img-slide01.png          ← imagem-fonte gerada pelo GPT (capa)
    img-slide04.png          ← imagem-fonte gerada pelo GPT (slide de impacto, se houver)
    slide-01.html            ← HTMLs de trabalho ficam aqui
    slide-02.html
    ...
    posts prontos/           ← PNGs finais publicáveis (o que você seleciona pra postar)
      slide-01.png
      slide-02.png
      ...
      slide-NN.png           ← slide de CTA copiado de CTA's/
