...
6. METADADOS DO BRIEFING
   ...
   - Origem da imagem: [IA | Drive] — definida na pergunta do gate (ver abaixo)
```
...
**Antes do gate, perguntar a origem da imagem** (aplica-se a carrossel e post estático; pular para vídeo/roteiro):

```
---
ORIGEM DA IMAGEM — como você quer as imagens deste conteúdo?

[1] Imagem IA → gerada via prompt + gpt-image2-unity. Costuma integrar melhor
    à copy do post (composição pensada como peça única).
[2] Foto real do Drive → acervo de obra real da Fast, com curadoria crítica
    (autenticidade que a IA não reproduz — desde que a foto tenha qualidade de peça).
[3] Mista → combina IA e foto real na mesma peça (ex.: capa IA + slides de obra real,
    ou foto real de fundo com elementos/composição gerados). Definir quais slides usam cada origem.
---
```

Nenhuma opção é default — **sempre apresentar as TRÊS lado a lado** (IA, foto real e mista) e aguardar a escolha. Registrar a resposta em **`Origem da imagem`** nos metadados e no `_aprovado.md` (na mista, anotar quais slides vêm de IA e quais do Drive).

Depois de definida a origem, exiba o gate:
